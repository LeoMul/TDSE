module tdse_fourier
    ...
    function euler_method(psi_array,k_array,v_array,delta_t) result(newpsi)
        complex*16,intent(in) :: psi_array(:)
        real * 8,intent(in) :: k_array(:), v_array(:) ,delta_t
        complex*16 :: dpsidt (size(psi_array)),newpsi(size(psi_array))
        dpsidt = get_time_derivative(psi_array,k_array,v_array)
        newpsi = psi_array + dpsidt * COMPLEX(delta_t,0.0_dp)
    end function euler_method


    function propagate_wave_function_fft(initial_psi,v_array,delta_x,delta_t,num_time_steps,every) result (psi_matrix)
        ...
        complex*16 :: psi_matrix(num_time_steps/every,size(v_array)),next_psi(size(initial_psi))
        complex * 16::current_psi(size(initial_psi)),prev_psi(size(initial_psi))

        k_array = 6.28318530718_dp*create_fftfreq(size(v_array),delta_x)
        psi_matrix(1,:) = initial_psi
        current_psi = first_time_step(initial_psi,k_array,v_array,delta_t)
        prev_psi = initial_psi


        do j = 3,num_time_steps
            next_psi = next_time_step(current_psi,prev_psi,k_array,v_array,delta_t)
            if ((modulo(j,every) == 0) .or. (every == 1)) then
                print*,j
 
                psi_matrix(j/every,:) = next_psi
            end if 
    ...
    subroutine write_out_for_gif(psi_matrix,x_array,every,delta_t)
        complex*16,intent(in) :: psi_matrix(:,:) 
        real * 8, intent(in) :: x_array(:) ,delta_t
        integer,intent(in) :: every
        ...
        do jj = 1,num_time_steps
            ...
            if ((modulo(jj,every) == 1 ) .or. (every == 1)) then
        ...
        do jj = 1,num_time_steps
            ...
            do j = 1,size(x_array)
                write(1,FMT,advance = "no") abs(psi_matrix(jj,j)) 
                write(1,fmt="(1x,a)",advance = "no") " "
            end do 
            write(1,*) " "
         end if
